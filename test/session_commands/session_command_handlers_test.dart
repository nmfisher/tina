...
Future<void> main() async {
  ...
  group('SessionCommandHandlers /timers (§9, §11)', () {
    ...
    test('empty service lists "no active timers."', () async {
      final handlers = SessionCommandHandlers(
        _FakeCtx(conversation: conv, timers: timers),
      );
      await handlers.dispatch('/timers');
    ...
    test(
      'list shows one dim row per timer plus the count and footer',
      () async {
        timers.set(
          TimerSpec(
            name: 'check-build',
        ...
        expect(
          host.styledMessages.where((m) => m.style == HostMessageStyle.dim),
          hasLength(3), // two rows + the footer
          reason: 'rows and footer are dim (§9 indicative shape)',
        );
        expect(host.messages.join(), contains('check-build'));
        expect(
          host.messages.join(),
          contains(
    ...
    test(
      '/timers show <name> prints interval, fires, state, instruction',
      () async {
        timers.set(
          TimerSpec(
            name: 'check-build',
    ...
    test('/timers show <unknown> warns and names the missing timer', () async {
      final handlers = SessionCommandHandlers(
        _FakeCtx(conversation: conv, timers: timers),
      );
    ...
    test(
      '/timers cancel <name> cancels; unknown name lists the actives',
      () async {
        timers.set(
          TimerSpec(
            name: 'check-build',
