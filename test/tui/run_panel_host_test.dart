...
void main() {
  ...
  late PanelHost panels;
  late RunPanelHost panelHost;
  ...
  setUp(() {
    ...
    panelHost = RunPanelHost(panels: panels, makeSinkHost: makeSinkHost);
  });

  tearDown(() async {
    panels.dispose();
    contentCoordinator.dispose();
  ...
  ({OpenedRunPanel handle, TuiConversationHost? sink, bool Function() closed})
  open({
    ...
    final handle = panelHost.openPanel(
      (label: label, id: conversationId, placement: PanelPlacement.sideColumn),
      installSink: (host) => sinkHost = host,
      onStop: () {},
      onClose: () => closed = true,
    );
    return (handle: handle, sink: sinkHost, closed: () => closed);
  }
  ...
  group('openPanel', () {
    ...
    test('flows the spec label and conversationId through untouched', () {
      final r = open(label: 'wf e2e [run 7f3a]', conversationId: 'wf-run-7f3a');
    ...
    test(
      'the panel content is a read-only extra that does not reserve input',
      () {
        final r = open();
        expect(
          r.handle.frame.reservesInput,
          isFalse,
    ...
    test(
      'first open splits the layout; an already-open second panel tiles',
      () {
        ...
        expect(
          initialHost.stayAttachedWhenInactive,
          isTrue,
  ...
  group('closePanel', () {
    ...
    test('an already-closed handle is a no-op (idempotent close)', () {
      ...
      expect(
        ...
        isFalse,
        reason:
            'the close action belongs to the caller; closePanel only '
            'tears down',
      );
    });
  ...
  group('read-only keys', () {
    ...
    test(
      'text is consumed with a one-time dim notice; navigation falls through',
      () {
        final handle = panelHost.openPanel(
          spec(),
          installSink: (_) {},
        ...
        expect(
          'input disabled'.allMatches(
            handle.host.chat.snapshotLines().join('\n'),
          ),
          hasLength(1),
        );
    ...
    test('scroll hooks drive the transcript scrollback and its badge', () {
      ...
      expect(
        handle.host.chat.debugScrollOffset,
        greaterThan(0),
      ...
      sink.newline();
      expect(
        handle.host.chat.newWhileScrolled,
        isNonZero,
