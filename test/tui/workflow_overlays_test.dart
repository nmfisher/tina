...
void main() {
  ...
  test('node-attr editor: Ctrl-S applies edits to the node', () async {
    ...
    canned.events = [CharInput(' X'), ControlKey(ControlCode.ctrlS)];
    final applied = await runNodeAttrEditor(
      screen: screen,
      editor: LineEditor(screen: screen),
      node: node,
  ...
  test(
    'node-attr editor: ctrl-s with no edits round-trips context/writes',
    () async {
      final screen = fakeScreen(columns: 80, lines: 24);
  ...
  test(
    'node-attr editor: an empty prompt never writes an empty attr key',
    () async {
      final screen = fakeScreen(columns: 80, lines: 24);
  ...
  test(
    'graph editor: open an existing graph then Esc (no changes) closes',
    () async {
      final screen = fakeScreen(columns: 80, lines: 24);
      final graph = parseDot('''
      digraph E {
  ...
  test(
    'double-Esc exits a dirty workflow without reopening discard confirmation',
    () async {
      ...
      final pending = runWorkflowEditor(
        ...
        editor: editor,
        graph: parseDot(
          'digraph E { start [shape=Mdiamond]; done [shape=Msquare]; start -> done }',
        ),
        name: null,
        pipeline: defaultPipeline,
  ...
  test(
    'graph editor: Ctrl-C with unsaved changes asks before discarding',
    () async {
      final screen = fakeScreen(columns: 80, lines: 24);
      final graph = parseDot('''
      digraph E {
