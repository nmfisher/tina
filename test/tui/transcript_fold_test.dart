...
void main() {
  ...
  void toolCall(String name, Map<String, dynamic> input, String output) {
    ...
    sink().toolComplete(ToolCompleteEvent(
      name,
      name,
      isError: false,
      result: output,
  ...
  group('foldTranscriptCommand', () {
    ...
    test('list numbers only foldable blocks, from 1, with the marker',
        () async {
    ...
    test('show reveals a folded block; show again reports it is open',
        () async {
      bash('ls lib');
    ...
    test('all folds every block and reports the count; show all reverses it',
        () async {
    ...
    test('a bad index names the valid range instead of folding', () async {
      ...
      for (final arg in ['0', '2', 'wat', '']) {
        ...
        expect(host.messages.single,
            'no block $arg — /blocks lists 1 foldable block.\n',
            reason: 'argument "$arg"');
        expect(
            sink().blocks.firstWhere((b) => b.canFold).folded, isTrue,
