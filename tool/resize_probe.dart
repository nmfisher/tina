...
void main() {
  ...
  final child = std.create(
    nc.PlaneOptions(y: 1, x: 1, rows: 38, cols: 76, name: 'probe'),
  )!;
  print(
    'before resize: absYX=${child.absYX()} dims=${child.dimy()}x${child.dimx()}',
  );
  child.resize(0, 0, 0, 0, 0, 0, 38, 118);
  print(
    'after  resize: absYX=${child.absYX()} dims=${child.dimy()}x${child.dimx()}',
  );
  child.resize(0, 0, 0, 0, 0, 0, 37, 118);
  print(
    'after  resize2: absYX=${child.absYX()} dims=${child.dimy()}x${child.dimx()}',
  );
  child.moveYX(1, 1);
