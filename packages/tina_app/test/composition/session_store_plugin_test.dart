...
void main() {
  test('buildAppComposition without a store override resolves the JSONL store '
    ...
    expect(comp.pluginScope?.lookup(sessionStoreServiceKey), same(comp.store));
  });

  test('an injected store stays the composition store and bypasses the session '
    ...
    expect(comp.pluginScope?.lookup(sessionStoreServiceKey), isNull);
  });

  test(
    'composition dispose closes the plugin-owned store exactly once',
    () async {
      var closed = 0;
