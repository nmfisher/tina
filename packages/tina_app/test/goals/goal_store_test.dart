...
void main() {
  group('GoalStore', () {
    ...
    test('set rejects empty and over-long text', () {
      ...
      expect(
        () => store.set('c1', 'x' * (GoalStore.maxTextLength + 1)),
        throwsArgumentError,
      );
      store.dispose();
    });
    ...
    test(
      'recordVerdict caps evidence and dedupes an unchanged verdict',
      () async {
        final store = GoalStore();
        ...
        expect(
          store.read('c1').status!.evidence.length,
    ...
    test('recordVerdict without a goal throws StateError (the clear race)', () {
      final store = GoalStore();
      expect(
        () => store.recordVerdict('c1', GoalVerdict.achieved, 'e'),
        throwsStateError,
      );
      store.dispose();
    });
    ...
    test(
      'clear removes the goal and fires only when something existed',
      () async {
        final store = GoalStore();
        var changes = 0;
        final sub = store.changes.listen((_) => changes++);
  ...
  group('GoalStore persistence (JSON + hydrate)', () {
    ...
    test(
      'hydrate restores, repaints once, clears authoritatively, never throws',
      () async {
        final store = GoalStore();
        var changes = 0;
        final sub = store.changes.listen((_) => changes++);
