...
void main() {
  ...
  setUp(() {
    ...
    File('${project.path}/.gitignore').writeAsStringSync('.tina/\n');
  ...
  void seedIndex(SidecarSummaryRepo repo, List<String> partition) {
    ...
    for (final dir in partition) {
      File(
        '${project.path}/.tina/summaries/${summarySlug(dir)}.md',
  ...
  test(
    'status on a first run: every dir stale, firstRun true, headSha set',
    () async {
  ...
  test('status reflects a recorded manifest as up-to-date', () async {
    ...
    final idx = _index();
    seedIndex(_repo(), _repo().defaultPartition());
  ...
  test(
    'status: the allocated layout IS the partition once any exists',
    () async {
      ...
      AllocationsStore.forProject(
        project.path,
      ).set(dir: 'lib/src', model: 'fast/fast-model');
  ...
  test(
    'status: no allocations → default partition, hasAllocations false',
    () async {
      final s = await _index().status();
  ...
  test(
    'status detects a partially-stale repo (only one changed dir)',
    () async {
  ...
  test(
    'status detects a deleted dir as stale+deleted after a removal commit',
    () async {
      final idx = _index();
      seedIndex(_repo(), _repo().defaultPartition());
