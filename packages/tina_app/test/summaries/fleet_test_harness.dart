...
({Directory project, Directory sidecarRoot, Directory tempRoot})
  ...
  git(project, ['init']);
  // `.tina` is tool-owned sidecar state (the summaries sidecar is its own Git
  // repository); a project commit must never sweep the nested repo into the
  // index, so keep it ignored like a real tina workspace would.
