...
import 'package:tina_app/src/workflows/default_workflow.dart';
...
void main() {
  group('resolveDefaultWorkflowName', () {
    ...
    test('null when the workflows dir is missing', () {
      expect(
        resolveDefaultWorkflowName(configured: null, workflowsDir: workflows),
        isNull,
      );
    });
    ...
    test('default.dot present -> "default"', () {
      ...
      expect(
        resolveDefaultWorkflowName(configured: '', workflowsDir: workflows),
        'default',
      );
    });
    ...
    test('configured "none" disables even when default.dot exists', () {
      ...
      write('default', 'digraph d {}');
      expect(
        resolveDefaultWorkflowName(configured: 'none', workflowsDir: workflows),
        isNull,
      );
    });
    ...
    test('configured name requires that file to exist', () {
      ...
      write('foo', 'digraph f {}');
      expect(
        resolveDefaultWorkflowName(configured: 'foo', workflowsDir: workflows),
        'foo',
      );
      expect(
        resolveDefaultWorkflowName(configured: 'bar', workflowsDir: workflows),
        isNull,
      );
    });
  });
  ...
  group('seedDefaultWorkflow', () {
    ...
    test('creates the dir + default.dot; second call is a no-op', () {
      ...
      expect(File(p.join(workflows.path, 'default.dot')).existsSync(), isTrue);
    ...
    test('seed parses and validates cleanly (no errors or warnings)', () {
      ...
      final source = File(
        p.join(workflows.path, 'default.dot'),
      ).readAsStringSync();
    ...
    test('seed graph has the full review-then-parallel-execute shape', () {
      ...
      expect(
        graph.node('plan_review_1')!.systemPrompt,
      ...
      expect(
        graph.outgoing('clarify').map((e) => e.to).toList(),
      ...
      final fanoutTargets = graph.outgoing('fanout').map((e) => e.to).toSet();
      expect(
        fanoutTargets,
    ...
    test('seed runs end-to-end: two reviews approve, work fans out, '
      ...
      final registry = NodeHandlerRegistry()
        ...
        ..register(
          'wait.human',
          HumanGateHandler(_UnexpectedGateInterviewer()),
        );
      ...
      expect(
        order.indexOf('plan_review_2'),
        greaterThan(order.indexOf('plan')),
      );
      expect(
        order.indexOf('plan_review_2'),
      ...
      expect(
        order.indexOf('fanout'),
        greaterThan(order.indexOf('plan_review_2')),
      );
      ...
      expect(
        order.indexOf('exec_reviewer'),
        greaterThan(order.indexOf('fanin')),
      );
      expect(
        backend.calls.firstWhere((c) => c.nodeId == 'exec_reviewer').preamble,
        allOf(
          contains('did chunk 1'),
          contains('did chunk 2'),
          contains('did chunk 3'),
        ),
      );
      // $input was expanded into the intake node's prompt.
      expect(
        backend.calls.firstWhere((c) => c.nodeId == 'intake').prompt,
        contains('fix the bug'),
      );
    });

    test(
      'clarify verdict routes through the human gate and back to a review',
      () async {
        seedDefaultWorkflow(workflows);
        final source = File(
          p.join(workflows.path, 'default.dot'),
        ...
        backend.perCall = (id, n) {
          if (id == 'plan_review_1' && n == 1) {
            return CodergenResult(
              'I need a decision from the user\nVERDICT: clarify',
        ...
        final review1Runs = store.nodes
            .where((n) => n.nodeId == 'plan_review_1')
            .length;
  ...
  group('ensureDefaultWorkflowUsable', () {
    ...
    test('missing file throws', () {
      expect(
        () => ensureDefaultWorkflowUsable(workflows, 'nope'),
    ...
    test('unparseable source throws', () async {
      ...
      await expectLater(
        ensureDefaultWorkflowUsable(workflows, 'bad'),
        throwsA(isA<DefaultWorkflowUnusable>()),
      );
    });
    ...
    test('validation error (no terminal node) throws', () async {
      ...
      await expectLater(
        ensureDefaultWorkflowUsable(workflows, 'noexit'),
        throwsA(isA<DefaultWorkflowUnusable>()),
      );
    });
  });
