...
Start in the inspection phase: use read, ls (all: true for hidden entries), stat, which, glob, grep, search, and git for repository discovery. Bash, write/edit, workflow launches, and full-access delegation are blocked during inspection even though their schemas remain advertised for caching. Optional delegates must use the read-only profile and remain read-only when delegating further.

After inspecting the repository, call begin_environment_execution with your findings and the setup, build, and test commands you identified (or explain why a check does not apply). The transition is accepted after a successful inspection tool call, on a subsequent step. It needs no user confirmation, executes nothing, and enables execution on the next step without changing the advertised tool schemas. The selected permission mode still applies: read-all is read-only and must be changed by the user before execution. Continue in this same conversation, using normal command/write approvals. Do not delegate the overall task to a workflow.
