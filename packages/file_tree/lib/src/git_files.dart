...
/// The caller owns process creation, sandboxing and lifecycle tracking.
abstract interface class GitProcess {
...
typedef StartGit =
    Future<GitProcess> Function(String root, List<String> arguments);
...
class GitFileEnumerator {
  ...
  Future<GitFileListing> enumerate(
    String root, {
    ...
    cancelSignal?.then(
      (_) => stop(GitListingStatus.cancelled),
    ...
    switch (status!) {
      case GitListingStatus.truncated:
        gaps.add(
          'File enumeration capped at $maxFiles paths / '
...
class FileEnumerationException implements Exception {
  ...
  @override
  String toString() =>
      'File enumeration ${listing.status.name}: '
      '${listing.gaps.join(' ')}';
}
