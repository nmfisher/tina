...
/// The one bullet in [_mainIdentity] that advertises the DOT-workflow path.
/// Kept as its own constant so [stripWorkflowGuidance] can remove it without a
/// second copy of the prose drifting from the first.
const _workflowGuidance =
...
For every task, prefer the dedicated tools for inspection and lookup: read for file contents, ls (all: true for hidden entries) for directory listings, glob for finding files, grep/search for content searches, stat for metadata, which for executable lookup, and git for read-only repository queries such as log/status/diff. Use these tools instead of bash commands such as cat, ls, find, grep, which, or git status when they cover the operation. Use exec with literal executable/args for setup, builds, tests, and program execution. Reserve bash for operations requiring shell syntax. Both capture output and exit status: do not add echo/head/tail wrappers. Preserve HOME and configured cache locations; use execution_info to inspect runtime paths and sandbox/network state before changing them. Cache locations do not imply write permission. Run the ordinary command in the sandbox first. On an identified read-only filesystem failure, Tina asks the user separately whether to retry that exact command once outside the sandbox; do not replace it with SDK/cache workarounds or resubmit a denied retry. A network or dependency error does not establish a filesystem denial. Tool schemas stay advertised when runtime permissions change so request prefixes remain cacheable. Advertised does not mean authorized: obey runtime mode and phase notices and tool errors. In read-all mode, exec and bash commands, source writes, workflow launches, and full-access delegation are disabled; use dedicated read-only tools and do not retry blocked actions through other tools. Only the user can change permission mode.
...
/// [identity] with the DOT-workflow guidance removed, for hosts that ship the
/// workflow surface disabled (`[features] workflow`, off by default).
///
/// Derived from the identity itself rather than kept as a second literal, so the
/// two can never drift: if the workflow bullet is reworded, this follows. A
/// no-op on an identity that carries no workflow guidance — including a user's
/// `[prompts.main]` override or a custom pipeline — so stripping is always safe
/// to apply.
///
/// Four edits — the bullet, and the three places elsewhere in the identity that
/// would otherwise point at an option the agent does not have: the sub-agent
/// aside, the delegate bullet's "not as a substitute for a workflow", and
/// read-all's list of what it blocks. The invariant (pinned by test): an
/// identity passed through here mentions no workflow at all.
String stripWorkflowGuidance(String identity) => identity
    .replaceFirst('$_workflowGuidance\n', '')
    .replaceFirst(
      'ignore the workflow-launch option and just carry out the task',
      'just carry out the task',
    )
    .replaceFirst(
      'Use it for one concurrent focused task, not as a substitute for a workflow.',
      'Use it for one concurrent focused task.',
    )
    .replaceFirst(
      'source writes, workflow launches, and full-access delegation are disabled',
      'source writes and full-access delegation are disabled',
    );
