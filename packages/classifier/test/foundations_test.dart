...
void main() {
  test(
    'explicit pre-dispatch rejections release reservations for queued work',
    () async {
      var calls = 0;
  ...
  test(
    'complete measured usage releases reservations for queued requests',
    () async {
      var calls = 0;
      final order = <Object?>[];
      final unit = JudgmentRequestBudget().estimate(req()) + 100;
  ...
  test('invalid batch is rejected atomically before any dispatch', () async {
    ...
    await expectLater(
      batch.run(List.filled(257, req())),
      throwsA(failure(JudgmentFailure.budgetExceeded)),
    );
    await expectLater(
      batch.run([req(), req('x' * 30000)]),
  ...
  test('counts complete UTF-8 JSON with overhead', () {
    ...
    expect(
      b.estimate(req('汉字👋')),
      utf8.encode(jsonEncode(req('汉字👋').toJson(model: b.model))).length + 77,
    );
  });
  test(
    'chunks preserve Unicode evidence, offsets and complete choice rubric',
    () {
      final b = JudgmentRequestBudget(maxInputTokens: 420, overheadTokens: 10);
      final choice = ChoiceQuestion(
        'where',
        instructions: 'Select',
      ...
      for (final text in [
        ...
        final chunks = b.chunkText(
          source: 'a.dart',
          text: text,
          questions: [choice],
        );
        expect(
          chunks.map((c) => (c.request.state.value as Map)['text']).join(),
          text,
        );
        var offset = 0;
  ...
  test('oversize rejected before creating HTTP client', () async {
    ...
    final service = TypeSafeJudgmentService(
      config: TypeSafeConfig(
        apiKey: 'key',
        requestBudget: JudgmentRequestBudget(
          maxInputTokens: 200,
          overheadTokens: 10,
        ),
      ),
  ...
  test('pre-cancelled starts nothing; overlapping batches rejected', () async {
    ...
    final cancelled = await batch.run([
      req(),
  ...
  test(
    'reported overrun halts dispatch preserving completed evidence',
    () async {
      var calls = 0;
      final batch = runner(
        Fake((r, c) async {
