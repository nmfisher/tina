...
/// candidate in order, but not necessarily contiguously. Bonuses reward
/// contiguous runs, word-boundary hits, and case-exact matches. Returns
/// null when no match is possible.
///
...
bool _isBoundary(String s, int i) {
  ...
  if (prev == 0x5F || prev == 0x2D) return true; // '_' '-'
...
/// Rank [candidates] against [query] using [fuzzyScore]. Returns a new list
/// containing only the matches, sorted best-first. The input is not mutated.
