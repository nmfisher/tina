...
/// `allocate_region` — give a directory its own region agent. Writes the
/// allocation and kicks off a summary generation for that directory
/// (fire-and-forget: the fleet runs in the background and the summary lands
/// when it finishes).
class AllocateRegionTool implements Tool {
  AllocateRegionTool(this._regions, this._index);

  final RegionRegistry _regions;
  final SummaryIndex _index;

  @override
  ToolSchema get schema => const ToolSchema(
        name: 'allocate_region',
        description: 'Give a directory its own region agent: it gets a '
            'persistent summary (generated in the background) and can be '
            'queried with query_region. Pass the repo-relative directory as '
            '`dir` (any nesting is fine). Optionally pass llm_provider + '
            'llm_model to give this region a dedicated fast model; otherwise '
            'it inherits the [regions] config default, then the main model.',
        inputSchema: {
          'type': 'object',
          'properties': {
            'dir': {
  ...
  Future<ToolResult> execute(
    ...
    // Fire-and-forget: generate just this region's summary. A blocking fleet
    // run would stall the main agent's turn for minutes.
    unawaited(_index.refresh(dirs: [dir]));
    return ToolResult('Allocated a region agent for "$dir"'
        '${_modelOverride(input) == null ? '' : ' (${_modelOverride(input)})'}. '
        'Its summary is being generated in the background; it will be ready '
        'shortly. Run `/index` later if it does not appear.');
  }
}
