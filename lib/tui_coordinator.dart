...
class TuiCoordinator {
    ...
    try {
        ...
        permissionAskerBuilder: (runSink) {
          ...
          return classifier == null
              ...
              : modeAwareAsker(
                  ...
                  fallback: WorkflowPermissionAsker(
                    sink: runSink,
                    screen: screen,
                    editor: editor,
                    attentionQueue: attentionQueue,
      ...
      if (manifest != null) {
        ...
        for (final meta in manifest.conversations) {
          if (meta.id == initialConversationId)
            continue; // active, already built
          try {
      ...
      Future<String?> renderImageToPanel(String path) async {
        ...
        final rgba = Uint32List.fromList(
        ...
        final BackendSurface? targetSurface = contentCoordinator.surfaceOf(
      ...
      pipeline.imageRenderer.coordinate(renderImageToPanel);
      ...
      scheduler
          .persistence = (job, {required meta, required parentConversationId}) async {
        await initialRecorder.ensureRegistered();
        final sessionId = initialRecorder.sessionId;
        final conversationId = await store.createConversationWithMeta(
          sessionId,
          meta,
        );
      ...
      void panelizeRestoredConversation(
        Conversation conv, {
      ...
      for (final conv in restoredPanels) {
        panelizeRestoredConversation(conv, parentId: restoredParentOf[conv.id]);
      ...
      Future<({String ref, ToolProfile profile})?> pickSpawnedTarget() async {
        ...
        final activeModelRef =
            activeConv.recorder?.meta?.model ??
      ...
      final pauseSub = pauseGate.onPause.listen((reason) async {
        ...
        try {
          final cont = await runSpendPauseDialog(
            screen: screen,
            editor: editor,
          );
      ...
      if (gitRoot != null &&
        ...
        coordinator.pendingGitignoreAsk = () async {
          final choice = await runListOverlay<String>(
            ...
            accent: 'cyan',
            body:
                'Tina stores session transcripts in .tina/sessions/ inside '
                'this repo '
      ...
      Future<TmuxExitChoice> _runTmuxExitDialog() async {
        ...
        final entries = [
          (display: 'Detach', value: TmuxExitChoice.detach),
      ...
      controller.detachTmux = () async {
        ...
        if (r.ok) {
          final attach = tmux.attachLine;
          initialHost.showMessage(
            'detached — the agent keeps running'
      ...
      app.pipeline.imageRenderer.coordinate(null);
      try {
  ...
  Future<RunOutcome> run({bool setupMode = false}) async {
    ...
    if (setupMode) {
      ...
      return wrote != null ? RunOutcome.setupWrote : RunOutcome.setupCancelled;
    }
  ...
  ExitContext _captureExitContext() {
    ...
    final saved =
        (recorder != null && recorder.isInitialized) || conv.history.isNotEmpty;
    return ExitContext(
...
({Screen screen, String? warning}) _createScreen(
  ...
  switch (config.backend) {
    ...
    case BackendChoice.notcurses:
      ...
      // create() succeeds.
      try {
        final nc = NotcursesBackend.create(
          io: io,
