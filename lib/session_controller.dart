...
class SessionController implements CommandContext {
  ...
  void _startTurn(Conversation s, String input) {
    // §7.2 attribution: a REAL fire (its exact prompt is in the set handed
    // out by [fireTimer]) acks its in-flight window here — before the run, so
    // the end-ack below lands on the right timer — and shows its dim origin
    // line. A typed look-alike is never in the set and passes untouched. The
    // prompt stays in the set (peeked, not removed) until the turn's end-ack
    // consumes it: the end-ack is keyed on membership, so removing it here
    // would leave every fire un-acked (§7.3 counters never move).
    if (_liveTimerFirePrompts.contains(input)) {
      final m = _timerFirePromptRe.firstMatch(input);
      if (m != null) {
        timers?.ackStarted(m.group(1)!);
        s.host.showMessage('[timer ${m.group(1)} #${m.group(2)} fired]\n',
            style: HostMessageStyle.dim);
      }
    }
  ...
  Future<void> _runTurn(Conversation s, String input) async {
    ...
    // Timer bookkeeping (§7.3): a REAL fire turn acks its end — aborted when
    // the operator cancelled it or the agent aborted — which is where the
    // service applies expiry, cap removal, and the §8 suspension. Attribution
    // is the exact-prompt set: a typed look-alike (or a real fire whose turn
    // was started before this controller instance saw it) never acks.
    final timers = this.timers;
    if (_liveTimerFirePrompts.remove(input) && timers != null) {
      final m = _timerFirePromptRe.firstMatch(input);
      if (m != null) {
        timers.ackFinished(
          m.group(1)!,
    ...
    // revisions and returns. Runs on every turn end so tool-driven set/cancel
    // mutations reach disk without waiting for a fire.
  ...
  void fireTimer(String name, int fireNumber) {
    ...
    final prompt = '[timer $name #$fireNumber] scheduled check. '
  ...
  static final RegExp _timerFirePromptRe =
      RegExp(r'^\[timer (.+) #(\d+)\] scheduled check\. Instruction: ');
      ...
      try {
        ...
        await TimerSidecarStore().write(
            TimerSidecarStore.sidecarPathFor(
                '$dir/${group.key}.jsonl', group.key),
            group.key,
            group.value);
      } catch (e) {
        active.host.showMessage('timer save failed: $e\n',
            style: HostMessageStyle.warning);
      }
    }
  ...
  Future<void> restoreTimerStateForResume() async {
    ...
    try {
      ...
      if (dir == null) return;
      path = TimerSidecarStore.sidecarPathFor('$dir/$sessionId.jsonl',
          sessionId);
    } catch (_) {
    ...
    if (restorable.length != saved.length) {
      ...
      try {
        ...
        host.showMessage('timer save failed: $e\n',
            style: HostMessageStyle.warning);
      }
    }
    ...
    _timerFlushedRevision = timers.revision; // restore bumped it; state is flushed
    for (final name in notRestored) {
      host.showMessage("timer '$name' not restored — 8-timer limit reached.\n",
          style: HostMessageStyle.warning);
    }
  }
  ...
  String _timerRestoreSummary(List<Map<String, Object?>> saved) {
    final b =
        StringBuffer('This session has ${saved.length} saved timer(s):');
